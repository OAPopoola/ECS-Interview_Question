...
store_cat = [{'item_name': 'Baked Beans', 'item_price': 0.99, 'item_discount': 15, 'bo_gof_discount_string': '2 1'},
             {'item_name': 'Biscuits', 'item_price': 1.20, 'item_discount': 4, 'bo_gof_discount_string': '1 1'},
             {'item_name': 'Sardines', 'item_price': 1.89, 'item_discount': 10, 'bo_gof_discount_string': '4 1'},
             {'item_name': 'Shampoo (Small)', 'item_price': 2.00, 'item_discount': 0, 'bo_gof_discount_string': '3 2'},
             {'item_name': 'Shampoo (Medium)', 'item_price': 2.00, 'item_discount': 0, 'bo_gof_discount_string': '2 1'},
             {'item_name': 'Shampoo (Large)', 'item_price': 1.89, 'item_discount': 0, 'bo_gof_discount_string': '1 1'}]
...
class ShoppingCart:
    ...
    def get_shopping_cart_discount(self) -> float:
        ...
        for item in self.cart:
            ...
            total_discount += (disc / 100) * item['qty'] * item['price']
        # get the second type of discount
        # This is the bo_gof type. For the purposes of this exercise we will assume
        ...
        for item in self.cart:
            ...
            if len(offerstr) != 0:
                ...
                if item['qty'] > (b + f):
                    ...
                    st = int(item['qty'] / (b + f))
                    total_discount += (item['price'] * st * f)
...
def lookup_item_discount(name: str) -> int:
    ...
    for item in store_cat:
        if item['item_name'] == name:
            return item['item_discount']


def lookup_item_bogof_discount(name: str) -> str:
    """This function returns the bo_gof discount string associated with the item in the shop catalog"""
    for item in store_cat:
        if item['item_name'] == name:
            return item['bo_gof_discount_string']
...
def main():
    ...
    cart1.add_item_to_cart(item)
    item = get_item_dict('Baked Beans', 0.99, 50)
    cart1.add_item_to_cart(item)
